...
class PlanetRenderer {
    constructor(canvas) {
        ...
        this.rotation   = 0;
        this.rotSpeed   = 0.003;   // radians per frame (visible rotation)
    ...
    render(planetState, biosphereState, evolutionState, civilizationState, intelligenceState) {
        ...
        this._drawBioluminescence(ctx, planetState);   // deep-ocean glow
        this._drawOceanSheen(ctx);
        this._drawContinents(ctx, planetState, biosphereState);
        this._drawMycelialGlow(ctx, intelligenceState);  // mycorrhizal network
        ...
        this._drawAuroraBorealis(ctx, planetState);    // polar auroras (outside clip, over halo)
        this._drawBiospherePulseRing(ctx, intelligenceState); // health pulse
        this._drawTippingAlerts(ctx, intelligenceState);      // tipping-point warnings
        this._drawHUDBadges(ctx, planetState, biosphereState, evolutionState, civilizationState, intelligenceState);
    }

    // ── lon/lat → canvas pixel (equirectangular, rotating) ───────────────────
    _project(lon, lat) {
        // Normalise lon relative to current rotation (0→360 maps to 0→2π)
        const rotDeg  = (this.rotation * 180 / Math.PI) % 360;
        let adjLon    = lon + rotDeg;
        // Wrap to [-180, 180]
        while (adjLon >  180) adjLon -= 360;
        while (adjLon < -180) adjLon += 360;

        // Simple orthographic-like projection: x from lon, y from lat
        const x = this.cx + (adjLon / 180) * this.radius;
        const y = this.cy - (lat  /  90) * this.radius;
        // Visibility: points on the far hemisphere (|adjLon| > 90) fade out
        const visible = Math.abs(adjLon) < 90;
        const fade    = Math.max(0, Math.cos(adjLon * Math.PI / 180));
        return { x, y, visible, fade };
    }

    // ── Space background ──────────────────────────────────────────────────────
    _drawSpace(ctx) {
        const g = ctx.createRadialGradient(this.cx, this.cy, this.radius * 1.2, this.cx, this.cy, Math.max(this.W, this.H) * 0.95);
    ...
    // ── Continents — realistic lat/lon polygon landmasses ────────────────────
    _drawContinents(ctx, planetState, biosphereState) {
        ...
        const temp        = parseFloat(planetState?.climate?.global_temp ?? 14);
        const desertification = Math.max(0, (temp - 16) * 0.04);   // warming dries tropics
        ...
        this._landmasses.forEach(land => {
            ...
            // Base biome colour
            let col = biomeColor[land.biome] || V3_PALETTE.temperate;

            // Apply climate modifiers: desertification, greenness
            ctx.fillStyle = col;
            ctx.fill();

            // Vegetation green tint on non-desert/tundra
            ctx.shadowBlur = 0;
            if (land.biome !== 'desert' && land.biome !== 'tundra' && greenness > 0.3) {
                ctx.fillStyle   = `rgba(30,180,50,${Math.min(0.35, 0.12 * greenness - desertification * 0.05)})`;
                ctx.fill();
            }
            // Desert heat tint
            if (desertification > 0.05 && land.biome === 'savanna') {
                ctx.fillStyle = `rgba(200,140,30,${desertification * 0.4})`;
                ctx.fill();
            }
    ...
    // ── Ice caps — realistic polar bands ─────────────────────────────────────
    _drawIceCaps(ctx, planetState) {
        const icePct  = parseFloat(planetState?.climate?.ice_fraction ?? 3.5);
        // Latitude at which ice edge sits (higher icePct → lower latitude)
        const iceEdgeLat = 90 - icePct * 15;  // ~67° at baseline, recedes with warming

        // Draw ice as a filled band above the ice-edge latitude (north) and below (south)
        const drawCap = (poleLat, sign) => {
            ctx.beginPath();
            // Walk longitude sweep drawing the ice edge
            for (let lon = -180; lon <= 180; lon += 6) {
                const edgePt = this._project(lon, sign * iceEdgeLat);
                const polePt = this._project(lon, sign * 89.5);
                if (lon === -180) {
            ...
            for (let lon = 180; lon >= -180; lon -= 6) {
                const polePt = this._project(lon, sign * 89.5);
                ctx.lineTo(polePt.x, polePt.y);
            }
            ctx.closePath();

            const g = ctx.createRadialGradient(this.cx, sign > 0 ? this.cy - this.radius * 0.85 : this.cy + this.radius * 0.85, 0,
                this.cx, sign > 0 ? this.cy - this.radius * 0.85 : this.cy + this.radius * 0.85, this.radius * 0.35);
            g.addColorStop(0,   V3_PALETTE.ice);
            g.addColorStop(0.7, V3_PALETTE.ice_glow);
            g.addColorStop(1,   'rgba(210,235,255,0.6)');
            ctx.fillStyle   = g;
    ...
    _drawOverlayLayer(ctx, planetState, biosphereState, civilizationState) {
        ...
        if (this.overlayMode === 'temperature') {
            const temp = parseFloat(planetState?.climate?.global_temp ?? 14);
            const t    = Math.max(0, Math.min(1, (temp - 5) / 30));
            col = `rgba(255,${Math.round(130 - t * 120)},0,${0.15 + t * 0.30})`;
        } else if (this.overlayMode === 'carbon') {
            const co2 = parseFloat(planetState?.climate?.atmosphere?.co2_ppm ?? 421);
            const t   = Math.max(0, Math.min(1, (co2 - 280) / 520));
            col = `rgba(255,130,0,${t * 0.38})`;
        } else if (this.overlayMode === 'civilization') {
            const p = parseFloat(civilizationState?.civilization?.biodiversity_pressure ?? 0) / 100;
            col = `rgba(220,50,50,${p * 0.42})`;
        } else {   // biosphere
            const soil = parseFloat(biosphereState?.fungal?.soil_health ?? 0.75);
            col = soil > 0.6 ? V3_PALETTE.health_good : soil > 0.3 ? V3_PALETTE.health_med : V3_PALETTE.health_bad;
        }
